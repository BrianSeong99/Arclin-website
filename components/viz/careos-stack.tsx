...
function Slabs({ layers, exploded, animate }: { layers: StackLayer[]; exploded: boolean; animate: boolean }) {
  const group = useRef<THREE.Group>(null);
  const refs = useRef<THREE.Group[]>([]);
  const boundary = useRef<THREE.Mesh>(null);
  // boundary sits above the first owner change from the bottom
  const bIdx = layers.findIndex((l, i) => i > 0 && l.owner !== layers[i - 1].owner);
  ...
  useFrame(({ clock }, dt) => {
    ...
    if (boundary.current && bIdx > 0) {
      boundary.current.position.y = (bIdx - 0.5) * gap;
      (boundary.current.material as THREE.MeshBasicMaterial).opacity = 0.05 + p.current * 0.25;
    }
  ...
  return (
    <group ref={group} rotation={[0, Math.PI / 4, 0]} position={[0, -1.45, 0]}>
      {layers.map((l, i) => (
        ...
        >
          ...
          <RoundedBox args={[2.05, 0.02, 2.05]} radius={0.01} position={[0, -0.06, 0]}>
            <meshStandardMaterial color={l.owner === "arclin" ? PINE : PINE_LIGHT} roughness={0.5} />
          </RoundedBox>
      ...
      {bIdx > 0 && (
        <mesh ref={boundary} position={[0, (bIdx - 0.5) * 0.16, 0]} rotation={[-Math.PI / 2, 0, 0]}>
...
export function CareOSStack({ layers, exploded, animate = true, className }: { layers: StackLayer[]; exploded: boolean; animate?: boolean; className?: string }) {
  return (
    <Canvas className={className} dpr={[1, 1.75]} flat frameloop={animate ? "always" : "demand"} orthographic camera={{ position: [5, 4.2, 5], zoom: 100, near: 0.1, far: 50 }} gl={{ alpha: true, antialias: true, powerPreference: "low-power" }} onCreated={({ camera }) => camera.lookAt(0, 0.2, 0)}>
      ...
      <Slabs layers={layers} exploded={exploded} animate={animate} />
    </Canvas>
  );
}
